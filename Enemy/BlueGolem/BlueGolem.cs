...
public class BlueGolem : MonoBehaviour
{
    ...
    private float _currentHp = 200 * PlayerStats.Instance.Level / 2;
    private float _maxHp = 200 * PlayerStats.Instance.Level / 2;
    ...
    {   // Ray를 쏠 포지션 설정
        ...
        RaycastHit2D groundRay = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y), Vector3.down, 3f, GroundLayer);
    ...
    { 
        isCanMove = false;
        PlayerStats.Instance.TotalKills += 1;
        Destroy(gameObject);
    }
